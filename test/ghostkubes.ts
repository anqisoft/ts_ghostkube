...
import { CACHE_DATA_ARRAY, CACHE_DATA_COUNT, SKIP_CACHE_DATA } from '../data/dataCache.ts';
...
function getMannerFromCubeInfo(inNoArray: number[], outNoArray: number[]): [string, number] {
	let manner = '';
	let pieceCount = 0;
	// let test = '';
	for (let i = 1; i <= 12; ++i) {
...
async function readFileByStreamSync(filename: string, fn: (line: string) => number, COUNT: number) {
	async function readFileByStream(filename: string): void {
		...
		let line: string;
		while (remainCount > 0 && (line = await bufReader.readString('\n')) != Deno.EOF) {
...
function getCubeForDrawing(no: number, CUBE_LINE: string, ACT_CELLS: string): CubeForDrawing {
	...
	for (let infoIndex = 0; infoIndex < ACT_CELL_COUNT; ++infoIndex) {
		...
		const feature: CellFeature = featureAndLayerIndex >= 6 ? CellFeature.Piece : CellFeature.Face;
		const layerIndex: number = featureAndLayerIndex % 6 + 1;

		//      3）1位：面序或片序——面序0-5，片序0-11，转1位十六进制
		const sixFaceAndFaceDirectionOrTwelveEdge = parseInt(CELL_INFO.substring(2, 3), 16);
		...
		const faceDirection: ConnectionRelation = parseInt(CELL_INFO.substring(3, 4), 16);
...
export async function done(
	...
	function addSourceManner(manner: string, pieceCount: number) {
		let finded = false;
		for (let sourceMannerIndex = 0; sourceMannerIndex < sourceMannerCount; ++sourceMannerIndex) {
	...
	for (let dataIndex = 0; dataIndex < DATA_COUNT; ++dataIndex) {
		...
		if (!SKIP_CACHE_DATA) {
			...
			for (let cubeIndex = 0; cubeIndex < CUBE_COUNT; ++cubeIndex) {
				...
				const [mannerRegExp, pieceCount] = getMannerFromCubeInfo(inNoArray, outNoArray);

				let find = false;
				for (let dataIndex = 0; dataIndex < CACHE_DATA_COUNT; ++dataIndex) {
					const CACHE_DATA = CACHE_DATA_ARRAY[dataIndex];
					const { mannerRegExp: CACHE_MANNER_REG_EXP, cube: CACHE_CUBE } = CACHE_DATA;
	...
	if (sourceMannerCount) {
		...
		await readFileByStreamSync(fileName, (mannerInfo: string) => {
			...
			for (let i = 0; i < sourceMannerCount; ++i) {
				...
				if (MANNER_REGEX.test(MANNER)) {
					const cubeNoArray = CUBE_NO_BILL.split('|').map((value) => parseInt(value));
					// log(SEARCH_MANNER, pieceCount, typeof cubeNoArray[0]);
					// log(MANNER, pieceCount, SEARCH_MANNER);
					if (SHOW_INFO_WHEN_PIECE_COUNT_EQUALS_OR_GREAT_THAN_TWO && pieceCount >= 2) {
						log(
							pieceCount,
					...
					if (cubeNo > 0) {
						...
						const ACT_CELLS =
							Deno.readTextFileSync(`../data/cubes/${CUBE_FILE_NO.toString().padStart(6, '0')}.txt`)
								.split(
									'\n',
		...
		if (okCountOfMannersFile < sourceMannerCount) {
			log(
				`[error]${sourceMannerCount - okCountOfMannersFile} items couldn't find in '${fileName}'.`,
			);
		}
	...
	for (let dataIndex = 0; dataIndex < DATA_COUNT; ++dataIndex) {
		...
		for (let cubeIndex = 0; cubeIndex < CUBE_COUNT; ++cubeIndex) {
			...
			const CUBE = cubeGoal.cube ? cubeGoal.cube : getCubeByManner(cubeGoal.manner);
			if (CUBE) {
	...
	NEW_CACHE_DATA_ARRAY.sort((prev, next) => prev.manner.localeCompare(next.manner));
