...
globalThis.onload = () => {
    ...
    let {
        page,
        pageTopSvg
    } = createNewPage(PAGE_WIDTH, PAGE_HEIGHT, SIDE_LENGTH, HALF_SIDE_LENGTH, ROW_COUNT, HOLE_LINE_CSS, HOLE_RADIUS);

    function appendNewPage() {
        const RESULT = createNewPage(PAGE_WIDTH, PAGE_HEIGHT, SIDE_LENGTH, HALF_SIDE_LENGTH, ROW_COUNT, HOLE_LINE_CSS, HOLE_RADIUS);
        page = RESULT.page;
        ...
        .forEach(({
            ...
            if (currentRowCount + rowCount + (JOIN_TO_LAST_CELL ? -1 : 0) > ROW_COUNT) {
                if (currentRowCount < ROW_COUNT) {
                    appendBottomElementsToPageTopSvg(pageTopSvg, SIDE_LENGTH, ROW_COUNT, currentRowCount, prevColCount, INNER_LINE_CSS);
                }
...
function appendBottomElementsToPageTopSvg(pageTopSvg, SIDE_LENGTH, ROW_COUNT, currentRowCount, prevColCount, INNER_LINE_CSS) {
    const Y1 = SIDE_LENGTH * currentRowCount;
    const Y2 = SIDE_LENGTH * ROW_COUNT;
    for (let colIndex = prevColCount - 5; colIndex < prevColCount; ++colIndex) {
        const X = SIDE_LENGTH * (colIndex + 1);
        SvgHelper.appendLine(pageTopSvg, INNER_LINE_CSS, X, X, Y1, Y2, null);
    }
    const X2 = SIDE_LENGTH * prevColCount;
...
function createNewPage(PAGE_WIDTH, PAGE_HEIGHT, SIDE_LENGTH, HALF_SIDE_LENGTH, ROW_COUNT, HOLE_LINE_CSS, HOLE_RADIUS) {
    ...
    for (let rowIndex = 0; rowIndex <= ROW_COUNT; ++rowIndex) {
        ...
        if (rowIndex < ROW_COUNT) {
            SvgHelper.appendCircle(pageTopSvg, HOLE_LINE_CSS, HALF_SIDE_LENGTH, Y + HALF_SIDE_LENGTH, HOLE_RADIUS, null);
        }
    }
    return {
        page,
