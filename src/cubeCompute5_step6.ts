 ...
 * src\cubeCompute5_step6.ts v0.0.1
...
const SOURCE_FILE_TOP_PATH = './step4/';
const GOAL_FILE_TOP_PATH = './step6_3/';
...
await (async () => {
	// step4/lines.txt => step6_3/lineToCubeNo.txt
	// compactLineInfo();
  ...
  // move cubes.txt to: _end/
	...
	function compactLineInfo_too_slowly_1() {
		...
		Deno.writeTextFileSync(
			...
			LINE_INFO_ARRAY.map((info, index) =>
				`${info}:${
					CUBE_NO_BEGIN_AND_END_ARRAY[index].map((SUB_ARRAY) => SUB_ARRAY.join('-')).join(',')
				}`
			).join('\n'),
		);
	}
	...
	function compactLineInfo_too_slowly_2() {
		...
		const INFO_ARRAY = Deno.readTextFileSync(`${SOURCE_FILE_TOP_PATH}linesBill.txt`).split('\r\n');
		showUsedTime('read linesBill.txt ok');
		// read linesBill.txt ok, used 508.46 milliseconds, or 0.508 seconds
		const DATA_ARRAY = Deno.readTextFileSync(`${SOURCE_FILE_TOP_PATH}lines.txt`).split('\n');
	...
	function compactLineInfo2() {
		...
		const DATA_ARRAY = Deno.readTextFileSync(`${GOAL_FILE_TOP_PATH}lines2.txt`).split('\r\n');
	...
	async function compactMannerFiles_tooSlowly_1() {
		...
		Deno.writeTextFileSync(
			...
			MANNER_ARRAY.map((MANNER, index) => `${MANNER}:${CUBE_NO_ARRAY[index]}`).join('\n'),
		);
    ...
    for(let outputIndex = 0; outputIndex < 4; ++outputIndex) {
      ...
      Deno.writeTextFileSync(
        ...
        MANNER_ARRAY.splice(0, QUARTER_COUNT).map((MANNER, index) => `${MANNER}\t${QUARTER_CUBE_NO_ARRAY[index]}`).join('\n'),
        APPEND_TRUE_FLAG
      );
    }
		showUsedTime(`output manners.txt`);
...
copySync('log.txt', 'log_step6.txt', { overwrite: true });
copySync('log.txt', `${GOAL_FILE_TOP_PATH}log${logFilenamePostfix}.txt`, { overwrite: true });
Deno.removeSync('./log.txt');
...
cls && deno lint %pwd%\cubeCompute5_step6.ts & deno fmt %pwd%\cubeCompute5_step6.ts

deno run --v8-flags=--max-old-space-size=20480 -A P:\anqi\Desktop\tech\ts\projects\203_ts_ghostkube\src\cubeCompute5_step6.ts

deno run --v8-flags=--max-old-space-size=20480 -A %pwd%\cubeCompute5_step6.ts

cls && deno run --v8-flags=--max-old-space-size=20480 -A %pwd%\cubeCompute5_step6.ts
