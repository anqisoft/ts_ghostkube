...
/**
 * <en_us>the result of the addition of cells to square body paper molds</en_us>
 * <zh_cn>准备添加单元格到正方体纸模的结果</zh_cn>
 * <zh_tw>準備添加單元格到正方體紙模的結果</zh_tw>
*/
...
/**
 * <en_us>The number of squares already (currently only counts the full five columns of paper molds, and there are only four or less cubes that are not generated by the paper mold)</en_us>
 * <zh_cn>已有正方体数量（当前仅统计了纸模五列齐全的情况，未生成纸模只有四列或更少列的正方体）</zh_cn>
 * <zh_tw>已有正方體數量（當前僅統計了紙模五列齊全的情況，未生成紙模只有四列或更少列的正方體）</zh_tw>
*/
const EXISTS_CUBE_COUNT = {
  row3: 20064,
  row5: 172641360, // TODO(@anqisoft) change it.
};

/**
 * <en_us>Deno cover file mark</en_us>
 * <zh_cn>deno覆盖文件标记</zh_cn>
 * <zh_tw>deno覆蓋文件標記</zh_tw>
*/
const OVER_WRITE_TRUE_FLAG = { overwrite: true };
/**
 * <en_us>Deno additional file mark</en_us>
 * <zh_cn>deno追加文件标记</zh_cn>
 * <zh_tw>deno追加文件標記</zh_tw>
*/
...
/**
 * <en_us>column number</en_us>
 * <zh_cn>列数</zh_cn>
 * <zh_tw>列數</zh_tw>
*/
const COL_COUNT = 5;
/**
 * <en_us>maximum serial number</en_us>
 * <zh_cn>最大列序号</zh_cn>
 * <zh_tw>最大列序號</zh_tw>
*/
...
/**
 * <en_us>Calculate the intermediate square, store × rows_5Cols</en_us>
 * <zh_cn>计算中间正方体，存入×rows_5cols</zh_cn>
 * <zh_tw>計算中間正方體，存入×rows_5cols</zh_tw>
 *
 * @param ROW_COUNT_ARRAY <en_us>array: number of paper mold rows</en_us><zh_cn>数组：纸模行数</zh_cn><zh_tw>數組：紙模行數</zh_tw>
 * @param GOAL_FILE_TOP_PATH <en_us>Save road root directory</en_us><zh_cn>保存路径根目录</zh_cn><zh_tw>保存路徑根目錄</zh_tw>
 * @param OUTPUT_CUT_MANNERS_ROW_BY_ROW <en_us>Whether to output the "shear scheme"</en_us><zh_cn>是否逐行输出“剪切方案”</zh_cn><zh_tw>是否逐行輸出“剪切方案”</zh_tw>
 */
function step1(
