 ...
 * src\cubeCompute5_step2.ts v0.0.1
...
import { emptyDirSync, ensureDirSync, moveSync, existsSync } from 'https://deno.land/std/fs/mod.ts'; // copySync,
...
// const SOURCE_FILE_TOP_PATH = 'P:/_cubes_240428/step1/';
const SOURCE_FILE_TOP_PATH = './step1/';
const GOAL_FILE_TOP_PATH = './step2/';
...
await (async () => {
	...
	function outputCubes() {
		...
		const GOAL_FILE_NAME = `${GOAL_FILE_TOP_PATH}${(++fileNo).toString().padStart(6, '0')}.txt`;
...
// Deno.removeSync('log_step2.txt');
// copy('log.txt', 'log_step2.txt');

// OK
copySync('log.txt', 'log_step2.txt', { overwrite: true });
Deno.removeSync('./log.txt');

// Deno.removeSync('./log_step2.txt');
// moveSync('log.txt', 'log_step2.txt');
// // Deno.removeSync('./log.txt');
...
cls && deno lint %pwd%\cubeCompute5_step2.ts && deno fmt %pwd%\cubeCompute5_step2.ts

deno run --v8-flags=--max-old-space-size=20480 -A P:\anqi\Desktop\tech\ts\projects\203_ts_ghostkube\src\cubeCompute5_step2.ts

deno run --v8-flags=--max-old-space-size=20480 -A %pwd%\cubeCompute5_step2.ts
