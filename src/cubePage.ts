...
globalThis.onload = () => {
  ...
  MANNERS.splice(0, 200).forEach(({ manner, cube }, cubeIndex) => {
    const {
      cells, // CellObject[][] = [];
      actCells, // CellObject[] = [];
    ...
    if (currentRowCount + rowCount + (JOIN_TO_LAST_CELL ? -1 : 0) > ROW_COUNT) {
      if (currentRowCount < ROW_COUNT) {
        // appendBottomElementsToPageTopSvg(pageTopSvg: SVGElement, SIDE_LENGTH: number, ROW_COUNT: number, currentRowCount: number, prevColCount: number, INNER_LINE_CSS: string): void
