 ...
 * src\cubeCompute5_step1.ts v0.0.1
 * deno 1.42.1 + VSCode 1.88.0
 *
 * <en_us>
 * Created on Sun Apr 28 2024 08:56:56
 * Feature:
 * </en_us>
 *
 * <zh_cn>
 * 创建：2024年4月28日 08:56:56
 * 功能：仅获取中间正方体。
 * </zh_cn>
 *
 * <zh_tw>
 * 創建：2024年4月28日 08:56:56
 * 功能：
 * </zh_tw>
 */

// https://www.cnblogs.com/livelab/p/14111142.html
import { copySync, emptyDirSync, ensureDirSync, moveSync, existsSync } from 'https://deno.land/std/fs/mod.ts'; // copy
...
const GOAL_FILE_TOP_PATH = './step1/';
	...
	function countMiddleCubes() {
		...
		function dealMiddleCubes() {
			...
			Deno.writeTextFileSync(
				`${MIDDLE_FILE_NAME_PREFIX}${(++middleFileNo).toString().padStart(3, '0')}.txt`,
...
// copy('log.txt', `${GOAL_FILE_TOP_PATH}log.txt`, { overwrite: true });
const GOAL_LOG_FILE_NAME = `${GOAL_FILE_TOP_PATH}log.txt`;
copySync('log.txt', 'log_step1.txt', { overwrite: true });

// Deno.removeSync(GOAL_LOG_FILE_NAME);
// moveSync('log.txt', GOAL_LOG_FILE_NAME);

copySync('log.txt', GOAL_LOG_FILE_NAME, { overwrite: true });
Deno.removeSync('log.txt');
...
cls && deno lint %pwd%\cubeCompute5_step1.ts && deno fmt %pwd%\cubeCompute5_step1.ts

cd /d P:\anqi\Desktop\tech\ts\_bak\203_ts_ghostkube\!important\data\240419A\

deno run --v8-flags=--max-old-space-size=20480 -A P:\anqi\Desktop\tech\ts\projects\203_ts_ghostkube\src\cubeCompute5_step1.ts
deno run --v8-flags=--max-old-space-size=20480 -A %pwd%\cubeCompute5_step1.ts
