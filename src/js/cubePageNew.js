...
/**
 * <en_us>en_us</en_us>
 * <zh_cn>单个正方体对应的纸模列数</zh_cn>
 * <zh_tw>zh_tw</zh_tw>
 */
const COL_COUNT_PER_CUBE = 5;
/**
 * <en_us>en_us</en_us>
 * <zh_cn>onload方法，对应于网页加载完成事件window.onload</zh_cn>
 * <zh_tw>zh_tw</zh_tw>
 */
globalThis.onload = () => {
    ...
    // 	<en_us>en_us</en_us>
    // 	<zh_cn>外线宽度。注意：若外边框的线宽太小，会导致“虽然有这些线，但看不到”的问题</zh_cn>
    // 	<zh_tw>zh_tw</zh_tw>
    ...
    const FACE_TEXT_COLOR = getPageParameterByName('face_text_color', DEFAULT_TEXT_COLOR).trim().length
    ...
    let { page, pageTopSvg } = createNewPage(PAGE_WIDTH, PAGE_HEIGHT, SIDE_LENGTH, HALF_SIDE_LENGTH, ROW_COUNT, COL_COUNT, HOLE_LINE_CSS, HOLE_RADIUS);
    ...
    SET_ARRAY.forEach(({ name, cubes }) => {
        // 240516
        // 	<en_us>add "skip" logic: 0 to skip one, the negative number is taken after the absolute value, and the corresponding quantity</en_us>
        // 	<zh_cn>增加“跳过”逻辑：0跳过一个，负数则取绝对值后跳过相应数量</zh_cn>
        // 	<zh_tw>增加“跳過”邏輯：0跳過一個，負數則取絕對值後跳過相應數量</zh_tw>
        let skipCount = 0;
        cubes.forEach((no, cubeIndex) => {
            ...
            const { firstRowActCellColIndexBill, // string = '';
            ...
            const svg = getSvg({
                id: ++svgId,
                setName: name,
                // 240516 use skipCount:
                // setNo: cubeIndex + 1, // number;
                setNo: cubeIndex + skipCount + 1,
                cubeNo: no,
...
function appendBottomElementsToPageTopSvg(pageTopSvg, SIDE_LENGTH, ROW_COUNT, currentRowCount, currentColCount, INNER_LINE_CSS) {
    ...
    for (let colIndex = currentColCount - COL_COUNT_PER_CUBE; colIndex < currentColCount; ++colIndex) {
