 ...
 *
 * <en_us>
  * Creation: 13:00:33 on May 8, 2024
  * Function: merge all steps
  * Note: You do not need to consider the "continuation" mode in the first step. The second step starts.
  * </en_us>
 *
 * <zh_cn>
 ...
 * <zh_tw>
  ...
  * 功能：合併所有步驟
  * 注意：第一步不用考慮“接續”模式，第二步開始，如果不是從2行開始，則自動進入“接續”模式
  * </zh_tw>
 */
import { copySync, emptyDirSync, ensureDirSync, existsSync, } from "https://deno.land/std/fs/mod.ts";
...
CellBorderLine, 
// CellBorderPosition,
CellFeature, CellObject, 
// COL_INDEX_ARRAY_MORE_THAN_THREE_ROW,
ConnectionRelation, convertSixFaceAndDirectionToSixFaceTwentyFourAngle, convertSixFaceTwentyFourAngleToSixFaceAndDirection, Cube, 
// FourDirectionCount,
// FourDirectionMaxIndex,
getCubeFromJson, getReversedRelation, getSixFaceTwentyFourAngleRelationTwelveEdge, getUsedTimeString, global_removed_middle_cube_count, 
// TwoCellRowColIndex,
Globals, log, logUsedTime, 
// showCubeCoreInfo,
...
// SimpleCell,
SimpleCube, SIX_FACE_AND_DIRECTION_RELATIONS, SixFaceTwentyFourAngle, 
// SixFaceTwentyFourAngleToTwelveEdge,
TwelveEdge, } from "./cubeCore.ts";
/**
 * <en_us>en_us</en_us>
 * <zh_cn>已有正方体数量（当前仅统计了纸模五列齐全的情况，未生成纸模只有四列或更少列的正方体）</zh_cn>
 * <zh_tw>zh_tw</zh_tw>
*/
...
/**
 * <en_us>en_us</en_us>
 * <zh_cn>deno覆盖文件标记</zh_cn>
 * <zh_tw>zh_tw</zh_tw>
*/
const OVER_WRITE_TRUE_FLAG = { overwrite: true };
/**
 * <en_us>en_us</en_us>
 * <zh_cn>deno追加文件标记</zh_cn>
 * <zh_tw>zh_tw</zh_tw>
*/
...
/**
 * <en_us>en_us</en_us>
 * <zh_cn>计算中间正方体，存入×rows_5cols</zh_cn>
 * <zh_tw>zh_tw</zh_tw>
 *
 * @param ROW_COUNT_ARRAY <en_us>en_us</en_us><zh_cn>数组：纸模行数</zh_cn><zh_tw>zh_tw</zh_tw>
 * @param GOAL_FILE_TOP_PATH <en_us>en_us</en_us><zh_cn>保存路径根目录</zh_cn><zh_tw>zh_tw</zh_tw>
 * @param OUTPUT_CUT_MANNERS_ROW_BY_ROW <en_us>en_us</en_us><zh_cn>是否逐行输出“剪切方案”</zh_cn><zh_tw>zh_tw</zh_tw>
 */
function step1(ROW_COUNT_ARRAY, GOAL_FILE_TOP_PATH = "./", OUTPUT_CUT_MANNERS_ROW_BY_ROW = false) {
    ...
    const GOAL_FILE_TOP_PATH_IS_NOT_CURRENT_PATH = GOAL_FILE_TOP_PATH.replace(/[.\/\\]/g, "").length;
    ...
    const MIDDLE_CUBE_COUNT_ARRAY = [0, 0, 0, 0, 0, 0];
    const CUT_MANNER_COUNT_ARRAY = [0, 0, 0, 0, 0, 0];
    const CUT_MANNER_ARRAY = [];
    ...
    function countByRowCount(ROW_COUNT) {
        ...
        const GOAL_FILE_PATH = `${GOAL_FILE_TOP_PATH}${ROW_COUNT}rows_${COL_COUNT}cols/`;
        ...
        function countMiddleCubes() {
            ...
            function countAndPushIfOk(simpleCube) {
                const cube = new Cube(++nextMiddleCubeNo, ROW_COUNT, simpleCube.colCount, CORE_ROW_INDEX, CORE_COL_INDEX, true);
                simpleCube.cells.forEach((sourceRow, rowIndex) => {
                    ...
                    sourceRow.forEach((sourceCell, colIndex) => {
                        ...
                        const goalCell = new CellObject(rowIndex, colIndex, COL_COUNT * rowIndex + colIndex);
                ...
                if (DEBUG.SHOW_CUT_MANNERS) {
                    const CUT_MANNER = cube.gridLines.map(({ xStart, xEnd, yStart, yEnd, lineStyle }) => `${xStart}${xEnd}${yStart}${yEnd}${lineStyle}`).join(",");
            ...
            function dealMiddleCubes() {
                ...
                Deno.writeTextFileSync(`${MIDDLE_FILE_NAME_PREFIX}${(++middleFileNo).toString().padStart(3, "0")}.txt`, MIDDLE_CUBE_ARRAY.map((cube) => JSON.stringify(cube)).join("\n"));
                MIDDLE_CUBE_ARRAY.length = 0;
            ...
            function countMiddleCube() {
                ...
                const HORIZONTAL_LINE_ARRAY = [];
                const VERTICAL_LINE_ARRAY = [];
                for (let horizontalLineIndex = 0; horizontalLineIndex < HORIZONTAL_LINE_COUNT; ++horizontalLineIndex) {
                    HORIZONTAL_LINE_ARRAY.push(2);
                }
                for (let verticalLineIndex = 0; verticalLineIndex < VERTICAL_LINE_COUNT; ++verticalLineIndex) {
                ...
                for (let i = 0; i < COUNT; ++i) {
                    ...
                    const EMPTY_CELL_POSITOIN_ARRAY = [];
                    ...
                    for (let rowLoop = 0; rowLoop < ROW_COUNT; ++rowLoop) {
                        if (EMPTY_CELL_POSITOIN_ARRAY.filter(([rowIndex]) => rowIndex === rowLoop)
                            .length < ROW_COUNT) {
                            ++notEmptyRowCount;
                        }
                    }
                    ...
                    for (let colLoop = 0; colLoop < COL_COUNT; ++colLoop) {
                        if (EMPTY_CELL_POSITOIN_ARRAY.filter(([, colIndex]) => colIndex === colLoop)
                            .length < COL_COUNT) {
                            ++notEmptyColCount;
                        }
                    }
            ...
            function prepareJoinCellToCube(HORIZONTAL_LINE_ARRAY, VERTICAL_LINE_ARRAY, cube, rowIndex, colIndex, addOrder, isCoreCell = false) {
                const { cells } = cube;
                // 	<en_us>If it has been added, return null</en_us>
                // 	<zh_cn>如已添加，则返回null</zh_cn>
                // 	<zh_tw>如已添加，則返回null</zh_tw>
                ...
                if (gridLines.filter((line) => line === CellBorderLine.OuterLine || line === CellBorderLine.CutLine).length === 4) {
                    return null;
                }
                ...
                // 	<en_us>Check if it is added, will it conflict</en_us>
                // 	<zh_cn>检查若添加进去，是否会冲突</zh_cn>
                // 	<zh_tw>檢查若添加進去，是否會衝突</zh_tw>
                let sixFaceTwentyFourAngle = SixFaceTwentyFourAngle.UpOriginal;
                let twelveEdge = TwelveEdge.NotSure;
                let hasError = false;
                let relationCellCount = 0;
                // 	<en_us>deliberate exchange order</en_us>
                // 	<zh_cn>故意交换顺序</zh_cn>
                // 	<zh_tw>故意交換順序</zh_tw>
                [gridLines[2], gridLines[3], gridLines[0], gridLines[1]].forEach((_line, relation) => {
                    if (hasError) {
                        return;
                    }
                    ...
                    if (oldCell.feature === CellFeature.Unknown ||
                    ...
                    if (oldCell.addOrder === RELATION_CELL_ADD_ORDER) {
                        ...
                        twelveEdge = getSixFaceTwentyFourAngleRelationTwelveEdge(OLD_CELL_SIX_FACE_TWENTY_FOUR_ANGLE, relation);
                    }
                    ++relationCellCount;
                    const currentSixFaceTwentyFourAngle = SIX_FACE_AND_DIRECTION_RELATIONS[OLD_CELL_SIX_FACE_TWENTY_FOUR_ANGLE][relation];
            ...
            function countByLines(HORIZONTAL_LINE_ARRAY, VERTICAL_LINE_ARRAY, EMPTY_CELL_POSITOIN_ARRAY) {
                LABLE_OUTER_LOOP: for (let coreColLoop = 0; coreColLoop < 5; ++coreColLoop) {
                    const coreColIndex = CORE_COL_INDEX_ORDER_ARRAY[coreColLoop];
                    if (EMPTY_CELL_POSITOIN_ARRAY.filter(([rowIndex, colIndex]) => {
                    ...
                    const NEXT_CELL_POSITION_ARRAY = [];
                    const CORE_CELL_INFO = prepareJoinCellToCube(HORIZONTAL_LINE_ARRAY, VERTICAL_LINE_ARRAY, cube, CORE_ROW_INDEX, CORE_COL_INDEX, 1, true);
                    ...
                    for (let addOrder = 1; addOrder < CELL_COUNT; ++addOrder) {
                        if (!NEXT_CELL_POSITION_ARRAY.length && addOrder > 1) {
                            if (DEBUG.SHOW_CUBE_WHEN_OK_IN_COUNT_BY_LINES) {
                                log(`\nok: ${addOrder}, core: ${CORE_ROW_INDEX}${CORE_COL_INDEX}\t${HORIZONTAL_LINE_ARRAY.join("")}\t${VERTICAL_LINE_ARRAY.join("")} => cells count: ${cube.cells.map((row) => row.filter((cell) => cell.feature === CellFeature.Face)
                                    .map((cell) => "1").join("")).join("").length}\n${cube.cells.map((row) => row.filter((cell) => cell.feature === CellFeature.Face)
                                    .map((cell) => `${cell.addOrder}: ${cell.rowIndex}${cell.colIndex}\t${cell.borderLines.join("")}\t(${cell.relatedInformationWhenAdding.rowIndex === -1
                                    ? " "
                                    : cell.relatedInformationWhenAdding.rowIndex},${cell.relatedInformationWhenAdding.colIndex === -1
                                    ? " "
                                    : cell.relatedInformationWhenAdding.colIndex}) ${cell.relatedInformationWhenAdding.relation}\t${cell.sixFace}+${cell.faceDirection.toString().padStart(3, " ")}${cell.twelveEdge.toString().padStart(3, " ")}${cell.sixFaceTwentyFourAngle.toString().padStart(3, " ")}`).join("\n")).join("\n")}`);
                            }
                            cube.count();
                            if (cube.isValid) {
                                countAndPushIfOk(cube);
                                // 	<en_us> found a suitable solution, exit the method directly!
                                /en_us>
                                    // 	<zh_cn>已找到合适的方案，直接退出方法！</zh_cn>
                                    // 	<zh_tw>已找到合適的方案，直接退出方法！
                                    < /zh_tw>;
                                return;
                            }
                            else {
                        ...
                        NEXT_CELL_POSITION_ARRAY.forEach(({ rowIndex, colIndex, gridLines, relatedInformationWhenAdding, sixFaceTwentyFourAngle, twelveEdge, }) => {
                            ...
                            const [sixFace, faceDirection] = convertSixFaceTwentyFourAngleToSixFaceAndDirection(sixFaceTwentyFourAngle);
                        ...
                        for (let rowIndex = 0; rowIndex < ROW_COUNT; ++rowIndex) {
                            for (let colIndex = 0; colIndex < COL_COUNT; ++colIndex) {
                                if (EMPTY_CELL_POSITOIN_ARRAY.filter(([emptyRowIndex, emptyColIndex]) => emptyRowIndex === rowIndex && emptyColIndex === colIndex)
                                    .length) {
                                    continue;
                                }
                                const result = prepareJoinCellToCube(HORIZONTAL_LINE_ARRAY, VERTICAL_LINE_ARRAY, cube, rowIndex, colIndex, addOrder + 1);
                                if (result !== null) {
    ...
    const LOG_FILE_NAME_IN_OTHER_PATH = `${GOAL_FILE_TOP_PATH}log${logFilenamePostfix}.txt`;
    ...
    if (LOG_FILE_NAME_IN_CURRENT_PATH !== LOG_FILE_NAME_IN_OTHER_PATH) {
        copySync(LOG_FILE_NAME, LOG_FILE_NAME_IN_OTHER_PATH, OVER_WRITE_TRUE_FLAG);
    }
    Deno.removeSync(LOG_FILE_NAME);
}
/**
 ...
 */
async function step2(ROW_COUNT_ARRAY, GOAL_FILE_TOP_PATH = "./", SOURCE_FILE_TOP_PATH = "./", OUTPUT_ALONE_FIRST_CUBE = false, OUTPUT_CUBE_PASS_CHECK_FACES_LAYER_INDEX = false, OUTPUT_FIX_HIDDEN_PIECES_DETAILS = false, OUTPUT_FIX_HIDDEN_PIECES = false, OUTPUT_MIDDLE_CUBE_TO_FIRST_NO = false, OUTPUT_CHECK_FACES_LAYER_INDEX_FAILED = false, OUTPUT_FIX_LONELY_FACE_OF_CUBE_AND_APPEND_IT = false, OUTPUT_FIX_LONELY_FACE_OF_CUBE = false) {
    let MiddleFileKind;
    ...
    for (let rowCountLoop = 2; rowCountLoop <= 5; ++rowCountLoop) {
        ...
        const SOURCE_FILE_PATH = `${SOURCE_FILE_TOP_PATH}${ROW_COUNT}rows_${COL_COUNT}cols/`;
        ...
        const GOAL_CUBE_FILE_PATH = `${GOAL_FILE_TOP_PATH}cubesOnlyFirstOfTwentyFour_${ROW_COUNT_FLAG}/`;
        ensureDirSync(GOAL_CUBE_FILE_PATH);
        emptyDirSync(GOAL_CUBE_FILE_PATH);
        let logFilenamePostfix = "";
        logFilenamePostfix = `_${STEP_AND_ROW_COUNT_FLAG}`;
        const DEBUG_FILE_PREFIX = (GOAL_FILE_TOP_PATH_IS_NOT_CURRENT_PATH ? "" : STEP_FLAG.concat("_"))
            .concat(ROW_COUNT_FLAG_PREFIX);
        log(`begin: ${(new Date()).toLocaleString()}`);
        const DATE_BEGIN = performance.now();
        const ALONE_FIRST_CUBE_PATH = `${GOAL_FILE_TOP_PATH}${STEP_AND_ROW_COUNT_FLAG}_29_aloneFirstCube/`;
        ...
        const FIX_HIDDEN_PIECES_DETAILS_PATH = `${GOAL_FILE_TOP_PATH}${STEP_AND_ROW_COUNT_FLAG}_22_fixHiddenPiecesDetails/`;
        ...
        const CUBE_PASSED_CHECK_FACES_LAYER_INDEX_PATH = `${GOAL_FILE_TOP_PATH}${STEP_AND_ROW_COUNT_FLAG}_21_cubePassedCheckFacesLayerIndex/`;
        ...
        const CHECK_FACES_LAYER_INDEX_FAILED_FILENAME = `${GOAL_FILE_TOP_PATH}${DEBUG_FILE_PREFIX}checkFacesLayerIndexFailed.txt`;
        const FIX_LONELY_FACE_OF_CUBE_FILENAME = `${GOAL_FILE_TOP_PATH}${DEBUG_FILE_PREFIX}fixLonelyFaceOfCube.txt`;
        const FIX_HIDDEN_PIECES_FILENAME = `${GOAL_FILE_TOP_PATH}${DEBUG_FILE_PREFIX}fixHiddenPieces.txt`;
        const FIX_LONELY_FACE_OF_CUBE_AND_APPEND_IT_FILENAME = `${GOAL_FILE_TOP_PATH}${DEBUG_FILE_PREFIX}fixLonelyFaceOfCubeAndAppendIt.txt`;
        const MIDDLE_CUBE_TO_FIRST_NO_FILENAME = `${GOAL_FILE_TOP_PATH}${DEBUG_FILE_PREFIX}middleCubeToFirstNo.txt`;
        ...
        const CHECK_FACES_LAYER_INDEX_FAILED_FILE_CONTENT_ARRAY = [];
        const FIX_LONELY_FACE_OF_CUBE_FILE_CONTENT_ARRAY = [];
        const FIX_HIDDEN_PIECES_FILE_CONTENT_ARRAY = [];
        const FIX_LONELY_FACE_OF_CUBE_AND_APPEND_IT_FILE_CONTENT_ARRAY = [];
        const MIDDLE_CUBE_TO_FIRST_NO_FILE_CONTENT_ARRAY = [];
        ...
        for (let middleFileKindIndex = 0; middleFileKindIndex < MIDDLE_FILE_KIND_COUNT; ++middleFileKindIndex) {
        ...
        await (async () => {
            ...
            function batchAppendCube(cubeOriginal) {
                ...
                cubeOriginal.actCells.forEach((cell) => {
                    const { rowIndex, colIndex, borderLines, sixFace, faceDirection, twelveEdge, } = cell;
                    ...
                    const cellRowColIndex = [rowIndex, colIndex];
                    switch (borderLines.filter((borderLine) => borderLine === CellBorderLine.InnerLine)
                        .length) {
                        case 2:
                        case 3:
                        case 4:
                                ...
                                frontFaceOptionalMannerArray.forEach((frontItem, frontIndex) => {
                                    backFaceOptionalMannerArray.forEach((backItem, backIndex) => {
                                        ...
                                        [
                                            ...
                                            const array = turpleArray[0];
                                            const item = turpleArray[1];
                                            const index = turpleArray[2];
                                            const USED_ROW_COL_INDEX = [];
                                            // const [firstRowIndex, firstColIndex, secondRowIndex, secondColIndex] =
                                            // 	item as OneOrTwoCellRowColIndex;
                                            const [currentSelectFirstRowIndex, currentSelectFirstColIndex, currentSelectSecondRowIndex, currentSelectSecondColIndex,] = item;
                                            const CURRENT_SELECT_ARE_TWO_PIECES = typeof currentSelectSecondRowIndex !==
                                                "undefined" &&
                                            ...
                                            array.slice(index, index + 1)
                                                ...
                                                const ARE_TWO_PIECES = typeof secondRowIndex !== "undefined" &&
                                                ...
                                                if (ARE_TWO_PIECES) {
                                                    if (CURRENT_SELECT_ARE_TWO_PIECES) {
                                                        ...
                                                        // );
                                                        if ((currentSelectSecondRowIndex ===
                                                            firstRowIndex &&
                                            ...
                                            const FIRST_ROW_COL_INDEX = `${currentSelectFirstRowIndex}_${currentSelectFirstColIndex}`;
                                            if (USED_ROW_COL_INDEX.indexOf(FIRST_ROW_COL_INDEX) ===
                                                -1) {
                                                USED_ROW_COL_INDEX.push(FIRST_ROW_COL_INDEX);
                                                cloned
                                                    .cells[currentSelectFirstRowIndex][currentSelectFirstColIndex]
                                                    .layerIndex = ++layerIndex;
                                            }
                                            if (CURRENT_SELECT_ARE_TWO_PIECES) {
                                                const SECOND_ROW_COL_INDEX = `${currentSelectSecondRowIndex}_${currentSelectSecondColIndex}`;
                                                if (USED_ROW_COL_INDEX.indexOf(SECOND_ROW_COL_INDEX) === -1) {
                                                    USED_ROW_COL_INDEX.push(SECOND_ROW_COL_INDEX);
                                                    cloned
                                                        .cells[currentSelectSecondRowIndex][currentSelectSecondColIndex]
                                                        .layerIndex = ++layerIndex;
                                                }
                                                sixFaceOutestCellArray.push(cloned
                                                    .cells[currentSelectSecondRowIndex][currentSelectSecondColIndex]);
                                                twelveEdges.forEach((edge) => {
                                                    removeFromPieces(currentSelectFirstRowIndex, currentSelectFirstColIndex);
                                                    removeFromPieces(currentSelectSecondRowIndex, currentSelectSecondColIndex);
                                                    function removeFromPieces(findRowIndex, findColIndex) {
                                                        let position = -1;
                                                        edge.pieces.forEach(([pieceRowIndex, pieceColIndex], pieceIndex) => {
                                        ...
                                        sixFaceOutestCellArray.forEach((OUTEST_CELL) => {
                                            OUTEST_CELL.borderLines.forEach((borderLine, borderLineIndex) => {
                                                if (borderLine !== CellBorderLine.InnerLine) {
                                                    const twelveEdgeIndex = getSixFaceTwentyFourAngleRelationTwelveEdge(OUTEST_CELL.sixFaceTwentyFourAngle, borderLineIndex);
                                        ...
                                        twelveEdges.forEach((oneEdge, edgeIndex) => {
                                            oneEdge.pieces.forEach((cellRowColIndex) => {
                                                ...
                                                const { rowIndex: relatedRowIndex, colIndex: relatedColIndex, } = pieceCell.relatedInformationWhenAdding;
                                        ...
                                        if (cloned.checkFacesLayerIndex()) {
                                            if (OUTPUT_CUBE_PASS_CHECK_FACES_LAYER_INDEX) {
                                                Deno.writeTextFileSync(`${CUBE_PASSED_CHECK_FACES_LAYER_INDEX_PATH}${(++cubePassCheckFacesLayerIndexFileNo)
                                                    .toString()}.js`, `const cube_${cubePassCheckFacesLayerIndexFileNo} = ${JSON.stringify(cloned)};\n// ${cloned.getManner()}`);
                                            }
            ...
            /**
             * <en_us>Check whether there are lonely noodles. If there is, the connected plugs or other directions will be stacked to the same position to the surface (both inside and outside); then handle the hidden surface</en_us>
             * <zh_cn>检查是否有孤面，若有，则将相连的插片或其它方向又叠到同一位置的插片转为面（内外皆可）；随后处理隐藏面</zh_cn>
             * <zh_tw>檢查是否有孤面，若有，則將相連的插片或其它方向又疊到同一位置的插片轉為面（內外皆可）；隨後處理隱藏面</zh_tw>
             *
             * @param cube Cube
             * <en_us>Cube that needs to be corrected</en_us>
             * <zh_cn>需修正的正方体</zh_cn>
             * <zh_tw>需修正的正方體</zh_tw>
             */
            function fixLonelyFaceOfCubeAndAppendIt(cube, recursiveTimes = 1) {
                const { cells, sixFaces, twelveEdges } = cube;
                const { cells: SOURCE_CUBE_CELLS } = cube;
                const PIECE_CELL_ARRAY = [];
                ...
                const LONELY_FACE_CELL_ARRAY = [];
                sixFaces.forEach((faces) => {
                    if (faces.length > 1) {
                        return;
                    }
                    const [firstRowIndex, firstColIndex, secondRowIndex] = faces[0];
                    // 	<en_us>exclude insertion (there is only one internal line, unrelated plug)</en_us>
                    // 	<zh_cn>排除插片（只有一条内联线，无关联插片）</zh_cn>
                    // 	<zh_tw>排除插片（只有一條內聯線，無關聯插片）</zh_tw>
                    if (typeof secondRowIndex !== "undefined") {
                        return;
                    }
                    const CELL = cells[firstRowIndex][firstColIndex];
                    const RELATION_PIECE_CELL_ARRAY = [];
                    const INNER_LINE_COUNT = CELL.borderLines.filter((borderLine) => borderLine === CellBorderLine.InnerLine).length;
                    PIECE_CELL_ARRAY.forEach((pieceCell) => {
                        if (pieceCell.relatedInformationWhenAdding.rowIndex ===
                            firstRowIndex &&
                    ...
                    if (INNER_LINE_COUNT - RELATION_PIECE_CELL_ARRAY.length === 1) {
                        ...
                        const SAME_FACE_PIECE_CELL_ARRAY = [];
                        PIECE_CELL_ARRAY.forEach((pieceCell) => {
                ...
                const { sameFacePieceCellArray, relationPieceCellArray } = FIRST_LONELY_FACE_CELL;
                if (OUTPUT_FIX_LONELY_FACE_OF_CUBE) {
                    // Deno.writeTextFileSync(
                    //   FIX_LONELY_FACE_OF_CUBE_FILENAME,
                ...
                const { layerIndex: LONELY_FACE_LAYER_INDEX, sixFace: LONELY_FACE_SIX_FACE, rowIndex: LONELY_FACE_ROW_INDEX, colIndex: LONELY_FACE_COL_INDEX, } = FIRST_LONELY_FACE_CELL;
                sameFacePieceCellArray.forEach((pieceCell) => {
                    const { rowIndex: PIECE_CELL_ROW_INDEX, colIndex: PIECE_CELL_COL_INDEX, } = pieceCell;
                    for (let inOutIndex = 0; inOutIndex < 2; ++inOutIndex) {
                        const cloned = cube.clone();
                        const { cells, sixFaces, twelveEdges } = cloned;
                        const CLONED_LONELY_FACE_CELL = cells[LONELY_FACE_ROW_INDEX][LONELY_FACE_COL_INDEX];
                        const CLONED_PIECE_CELL = cells[PIECE_CELL_ROW_INDEX][PIECE_CELL_COL_INDEX];
                        CLONED_PIECE_CELL.feature = CellFeature.Face;
                        const PIECES = twelveEdges[SOURCE_CUBE_CELLS[PIECE_CELL_ROW_INDEX][PIECE_CELL_COL_INDEX].twelveEdgeIndex].pieces;
                ...
                relationPieceCellArray.forEach((pieceCell) => {
                    ...
                    for (let inOutIndex = 0; inOutIndex < 2; ++inOutIndex) {
                        ...
                        const OLD_CELL_ARRAY = [];
                        let maxLayerIndex = 0;
                        cells.filter((cellRow) => cellRow.forEach((cell) => {
                            if (cell.feature !== CellFeature.Face ||
            ...
            /**
             * <en_us>Turn the hidden plug -in to the surface (both inside and outside)</en_us>
             * <zh_cn>将隐藏插片转为面（内外皆可）</zh_cn>
             * <zh_tw>將隱藏插片轉為面（內外皆可）</zh_tw>
             *
             * @param cube
             * <en_us>Cube that needs to be corrected</en_us>
             * <zh_cn>需修正的正方体</zh_cn>
            ...
            function fixHiddenPiecesOfCubeAndAppendIt(cube) {
                ...
                function getCellByCellIndex(cellIndex) {
                    const FILTERED = CELL_ARRAY.filter((cell) => cell.cellIndex === cellIndex);
                    return FILTERED.length ? FILTERED[0] : undefined;
                }
                twelveEdges.forEach((twelveEdge) => {
                    const REMOVE_INDEX_ARRAY = [];
                    twelveEdge.pieces.forEach(([pieceRowIndex, pieceColIndex], pieceIndex) => {
                        const PIECE_CELL = cells[pieceRowIndex][pieceColIndex];
                        const { sixFace: PIECE_CELL_SIX_FACE, } = PIECE_CELL;
                        const { rowIndex: PIECE_CELL_RELATION_CELL_ROW_INDEX, colIndex: PIECE_CELL_RELATION_CELL_COL_INDEX, relation: PIECE_CELL_RELATION, } = PIECE_CELL.relatedInformationWhenAdding.rowIndex === -1
                            ? cube.getCoreCellReserveRelatedInformation()
                            : PIECE_CELL.relatedInformationWhenAdding;
                        const PIECE_CELL_REVERSED_RELATION = getReversedRelation(PIECE_CELL_RELATION);
                        // console.log({
                        ...
                        // });
                        const PIECE_CELL_RELATION_CELL = cells[PIECE_CELL_RELATION_CELL_ROW_INDEX][PIECE_CELL_RELATION_CELL_COL_INDEX];
                        const PIECE_CELL_RELATION_CELL_SIX_FACE = PIECE_CELL_RELATION_CELL.sixFace;
                        const PIECE_CELL_RELATION_CELL_LAYER_INDEX = PIECE_CELL_RELATION_CELL.layerIndex;
                        const SAME_FACE_CELL_INFO_ARRAY = sixFaces[PIECE_CELL_RELATION_CELL_SIX_FACE];
                        const SAME_FACE_CELL_INFO_COUNT = SAME_FACE_CELL_INFO_ARRAY.length;
                        let hasOuterFace = false;
                        for (let sameFaceCellInfoIndex = 0; sameFaceCellInfoIndex < SAME_FACE_CELL_INFO_COUNT; ++sameFaceCellInfoIndex) {
                            const [firstRowIndex, firstColIndex, secondRowIndex, secondColIndex,] = SAME_FACE_CELL_INFO_ARRAY[sameFaceCellInfoIndex];
                            ...
                            if (firstCell.layerIndex > PIECE_CELL_RELATION_CELL_LAYER_INDEX &&
                                ...
                                getCellByCellIndex(firstCell.getConnectedCellIndexByRelation(PIECE_CELL_REVERSED_RELATION))?.feature === CellFeature.Face) {
                                hasOuterFace = true;
                                break;
                            }
                            if (typeof secondRowIndex !== "undefined" &&
                                ...
                                if (secondCell.layerIndex >
                                    ...
                                    secondCell.borderLines[PIECE_CELL_REVERSED_RELATION] ===
                                        CellBorderLine.InnerLine &&
                                    getCellByCellIndex(secondCell.getConnectedCellIndexByRelation(PIECE_CELL_REVERSED_RELATION))?.feature === CellFeature.Face) {
                                    hasOuterFace = true;
                                    break;
                                }
                            }
                        ...
                        if (hasOuterFace) {
                            ...
                            // 	<en_us>will cause negative numbers when compressing positive data</en_us>
                            // 	<zh_cn>会导致压缩正方体数据时出现负数</zh_cn>
                            // 	<zh_tw>會導致壓縮正方體數據時出現負數</zh_tw>
                            // PIECE_CELL.layerIndex = 0;
                            PIECE_CELL.layerIndex = 1;
                            ++hidePieceCount;
                        }
                    });
                    REMOVE_INDEX_ARRAY.toReversed().forEach((index) => twelveEdge.pieces.splice(index, 1));
                });
                if (hidePieceCount) {
            ...
            function outputCubes() {
                ...
                const GOAL_FILE_NAME = `${GOAL_CUBE_FILE_PATH}${ROW_COUNT_FLAG_PREFIX}${(++fileNo).toString().padStart(6, "0")}.txt`;
                Deno.writeTextFileSync(GOAL_FILE_NAME, CUBES.map((cube) => JSON.stringify(cube)).join("\n"));
                CUBES.length = 0;
        ...
        showUsedTime("end");
        log(`end: ${(new Date()).toLocaleString()}`);
        logUsedTime(`${STEP_AND_ROW_COUNT_FLAG}: Total`, performance.now() - DATE_BEGIN);
        const LOG_FILE_NAME_IN_CURRENT_PATH = `log_${STEP_FLAG}.txt`;
        const LOG_FILE_NAME_IN_OTHER_PATH = `${GOAL_FILE_TOP_PATH}log${logFilenamePostfix}.txt`;
        copySync(LOG_FILE_NAME, LOG_FILE_NAME_IN_CURRENT_PATH, OVER_WRITE_TRUE_FLAG);
        if (LOG_FILE_NAME_IN_CURRENT_PATH !== LOG_FILE_NAME_IN_OTHER_PATH) {
    ...
    GLOBAL_LOG_CONTENT_ARRAY.push(getUsedTimeString("Total", performance.now() - DATE_BEGIN));
    Deno.writeTextFileSync(`log_${STEP_FLAG}.txt`, GLOBAL_LOG_CONTENT_ARRAY.join("\n"));
}
/**
 ...
 */
async function step3(ROW_COUNT_ARRAY, GOAL_FILE_TOP_PATH = "./", SOURCE_FILE_TOP_PATH = "cubesOnlyFirstOfTwentyFour", OUTPUT_FULL_CUBE = false, OUTPUT_READ_CUBE = false, SHOW_GET_CLONED_CUBE_BY_MANNER_INDEX_DETAIL = false, MINI_OUTPUT = false) {
    const GOAL_FILE_TOP_PATH_IS_NOT_CURRENT_PATH = GOAL_FILE_TOP_PATH.replace(/[.\/\\]/g, "").length;
    if (GOAL_FILE_TOP_PATH_IS_NOT_CURRENT_PATH) {
        ensureDirSync(GOAL_FILE_TOP_PATH);
        emptyDirSync(GOAL_FILE_TOP_PATH);
    ...
    const MANNER_TO_CUBE_MAP_ARRAY = [];
    const MANNER_VALUE_ARRAY = [];
    const MANNER_VALUE_SET = new Set();
    const CURRENT_ROW_COUNT_MANNER_ARRAY = [];
    ...
    for (let rowCountLoop = 2; rowCountLoop <= 5; ++rowCountLoop) {
        ...
        const ROW_COUNT_FLAG = `${ROW_COUNT}rows`;
        const ROW_COUNT_FLAG_PREFIX = `${ROW_COUNT_FLAG}_`;
        const ROW_COUNT_FLAG_POSTFIX = `_${ROW_COUNT_FLAG}${MINI_OUTPUT_FLAG_POSTFIX}`;
        const STEP_AND_ROW_COUNT_FLAG = `${STEP_FLAG}_${ROW_COUNT_FLAG}`;
        ...
        log(`begin: ${(new Date()).toLocaleString()}`);
        const DATE_BEGIN = performance.now();
        const READ_CUBE_PATH = `${GOAL_FILE_TOP_PATH}${STEP_AND_ROW_COUNT_FLAG}_31_readCube/`;
        if (OUTPUT_READ_CUBE) {
            ensureDirSync(READ_CUBE_PATH);
            emptyDirSync(READ_CUBE_PATH);
        }
        const FULL_CUBE_PATH = `${GOAL_FILE_TOP_PATH}${STEP_AND_ROW_COUNT_FLAG}_39_fullCube/`;
        if (OUTPUT_FULL_CUBE) {
        ...
        await (async () => {
            ...
            const CUBES = [];
            const CUBE_GOAL_FILE_PATH = `${GOAL_FILE_TOP_PATH}cubes${ROW_COUNT_FLAG_POSTFIX}/`;
            if (!MINI_OUTPUT) {
                ensureDirSync(CUBE_GOAL_FILE_PATH);
                emptyDirSync(CUBE_GOAL_FILE_PATH);
            }
            const MANNER_CUBES_GOAL_FILE_PATH = `${GOAL_FILE_TOP_PATH}manners${ROW_COUNT_FLAG_POSTFIX}/`;
            if (!MINI_OUTPUT) {
                ensureDirSync(MANNER_CUBES_GOAL_FILE_PATH);
                emptyDirSync(MANNER_CUBES_GOAL_FILE_PATH);
            }
            // 	<en_us>2 lines 5 columns to clear the file first, and other situations use additional mode (if there are data before, not empty)</en_us>
            // 	<zh_cn>2行5列时先清除文件，其它情况则使用追加模式（如果之前有数据，不清空）</zh_cn>
            // 	<zh_tw>2行5列時先清除文件，其它情況則使用追加模式（如果之前有數據，不清空）</zh_tw>
            const CUBE_LINES_FILE_NAME = `${GOAL_FILE_TOP_PATH}lines${MINI_OUTPUT_FLAG_POSTFIX}.txt`;
            ...
            for await (const dirEntry of Deno.readDir(SOURCE_FILE_PATH)) {
                ...
                if (stats.isFile) {
                    ...
                    if (MINI_OUTPUT) {
                        ...
                        for (let i = 0; i < COUNT; ++i) {
                            MANNER_ARRAY.push(MANNER_VALUE_ARRAY[CURRENT_ROW_COUNT_MANNER_ARRAY[i]]);
                        }
                        Deno.writeTextFileSync(`${GOAL_FILE_TOP_PATH}mannerToCubeMap${ROW_COUNT_FLAG_POSTFIX}.txt`, MANNER_ARRAY.join("\n").concat("\n"), APPEND_TRUE_FLAG);
                        CURRENT_ROW_COUNT_MANNER_ARRAY.length = 0;
                        Deno.writeTextFileSync(`${GOAL_FILE_TOP_PATH}mannerToCubeMap.txt`, MANNER_TO_CUBE_MAP_ARRAY.join("\n").concat("\n"), APPEND_TRUE_FLAG);
                        Deno.writeTextFileSync(`${GOAL_FILE_TOP_PATH}manner.txt`, MANNER_VALUE_ARRAY.join("\n").concat("\n"), APPEND_TRUE_FLAG);
                        mannerValueCount = 0;
                        CURRENT_ROW_COUNT_MANNER_ARRAY.length = 0;
            ...
            function batchAppendCubeOneToTwentyFour(cube) {
                ...
                const { sixFaces: OLD_SIX_FACES, twelveEdges: OLD_TWELVE_EDGES, cells: OLD_CELLS, no: START_NO, rowCount, colCount, gridLines, } = cube;
                ...
                const MAX_ADD_ORDER = cube.actCells.map((cell) => cell.addOrder).sort((prev, next) => prev - next).reverse()[0];
                const CORE_CELL_IS_PIECE = OLD_CELLS[CORE_ROW_INDEX][CORE_COL_INDEX].feature ===
                    CellFeature.Piece;
                for (let mannerIndex = 0; mannerIndex < MANNER_COUNT; ++mannerIndex) {
                    const cloned = getClonedCubeByMannerIndex(cube, mannerIndex, OLD_SIX_FACES, 
                    // JSON.parse(JSON.stringify(cube.sixFaces)),
                    OLD_TWELVE_EDGES, 
                    // OLD_CELLS,
                    MAX_ADD_ORDER, CORE_CELL_IS_PIECE);
            ...
            function appendCube(cube) {
                if (MINI_OUTPUT) {
                    ...
                    // );
                    // 20240513
                    const { no, rowCount, actCells, lines, } = cube;
                    // ${COL_COUNT}
                    MANNER_TO_CUBE_MAP_ARRAY.push(`${MANNNER}\t${no}:${rowCount}_${actCells.map((cell) => {
                        const { layerIndex, 
                        // relation,
                        relatedInformationWhenAdding: { relation }, feature, sixFace, faceDirection, twelveEdge, rowIndex, colIndex, } = cell;
                        // const relation = relatedInformationWhenAdding;
                        return `${rowIndex}${colIndex}${relation}${layerIndex}${feature}${sixFace}${faceDirection},${twelveEdge}`;
                    }).join("|")}_${lines}`);
                    return;
                ...
                if (OUTPUT_READ_CUBE) {
                    Deno.writeTextFileSync(`${READ_CUBE_PATH}output.${CUBE_NO}.txt`, `${cube.getManner()}\n${JSON.stringify(cube.sixFaces)}\n${JSON.stringify(cube.twelveEdges)}`);
                    // Deno.writeTextFileSync(
            ...
            function outputCubes() {
                ...
                Deno.writeTextFileSync(`${MANNER_CUBES_GOAL_FILE_PATH}${filenamePostfix}`, MANNER_CUBES_ARRAY.map((noArray, index) => `${MANNER_ARRAY[index]}:[${noArray.join(",")}]`)
                    .join("\n"));
                ...
                try {
                    ...
                    CUBES.forEach((cube) => {
                        ...
                        actCells.forEach((cell) => {
                            const { cellIndex, layerIndex, feature, sixFace, faceDirection, rowIndex, colIndex, addOrder, } = cell;
                            let { twelveEdge } = cell;
                            ...
                            if (FEATURE_AND_LAYERINDEX.toString(16).indexOf("-") > -1) {
                                log(`${FEATURE_AND_LAYERINDEX} => ${FEATURE_AND_LAYERINDEX.toString(16)} <= ${FEATURE_AND_LAYERINDEX} <= ${feature}, ${layerIndex}`);
                            }
                            if (feature === 2) {
                                code += sixFace;
                            }
                                ...
                                if (typeof twelveEdge === "undefined") {
                                    ...
                                    if (relatedInformationWhenAdding.rowIndex === -1 &&
                                        ...
                                        const RELATION_CELL = actCells.filter((o) => o.addOrder === FIND_ADD_ORDER &&
                                            o.relatedInformationWhenAdding.rowIndex === rowIndex &&
                                            o.relatedInformationWhenAdding.colIndex === colIndex)[0];
                                        const relation = RELATION_CELL.relatedInformationWhenAdding.relation;
                                        twelveEdge = getSixFaceTwentyFourAngleRelationTwelveEdge(RELATION_CELL.sixFaceTwentyFourAngle, getReversedRelation(relation));
                                    }
                                    else {
                ...
                //   APPEND_TRUE_FLAG,
                // );
                Deno.writeTextFileSync(CUBE_LINES_FILE_NAME, (existsSync(CUBE_LINES_FILE_NAME) ? "\n" : "").concat(cubeLinesFileContent), APPEND_TRUE_FLAG);
                cubeLinesFileContent = "";
                CUBES.length = 0;
            }
            function getClonedCubeByMannerIndex(cube, mannerIndex, OLD_SIX_FACES, OLD_TWELVE_EDGES, MAX_ADD_ORDER, CORE_CELL_IS_PIECE) {
                const { coreRowIndex: CORE_ROW_INDEX, coreColIndex: CORE_COL_INDEX } = cube;
                const cloned = cube.clone();
                const { cells, actCells, sixFaces, twelveEdges } = cloned;
                const CELL_ARRAY = cloned.getCellArray();
                const [CORE_CELL_SIX_FACE, CORE_CELL_FOUR_DIRECTION] = convertSixFaceTwentyFourAngleToSixFaceAndDirection(mannerIndex);
                ...
                const NEW_SIX_FACE_ARRAY = SHOW_GET_CLONED_CUBE_BY_MANNER_INDEX_DETAIL
                    ? []
                    : undefined;
                ...
                for (let addOrder = 2; addOrder <= MAX_ADD_ORDER; ++addOrder) {
                    ...
                    //   )
                    // );
                    CELL_ARRAY.filter((cell) => cell.addOrder === addOrder).forEach((cell) => {
                        ...
                        //     ][RELATION],
                        //   );
                        const newSixFaceTwentyFourAngle = SIX_FACE_AND_DIRECTION_RELATIONS[sixFaceTwentyFourAngle][RELATION];
                        const [newSixFace, newFaceDirection] = convertSixFaceTwentyFourAngleToSixFaceAndDirection(newSixFaceTwentyFourAngle);
                        cell.sixFace = newSixFace;
                        cell.faceDirection = newFaceDirection;
                        cell.twelveEdge = getSixFaceTwentyFourAngleRelationTwelveEdge(sixFaceTwentyFourAngle, RELATION);
                        if (SHOW_GET_CLONED_CUBE_BY_MANNER_INDEX_DETAIL) {
                ...
                if (CORE_CELL_IS_PIECE) {
                    ...
                    // )[0];
                    // const RELATED_CELL = cells[rowIndex][colIndex];
                    const RELATED_CELL = CELL_ARRAY.filter((cell) => cell.addOrder === 2)[0];
                    const { sixFaceTwentyFourAngle } = RELATED_CELL;
                    ...
                    CORE_CELL.twelveEdge = getSixFaceTwentyFourAngleRelationTwelveEdge(sixFaceTwentyFourAngle, REVERSED_RELATION);
                    // TODO(@anqisoft) delete
                    const [newSixFace, newFaceDirection] = convertSixFaceTwentyFourAngleToSixFaceAndDirection(SIX_FACE_AND_DIRECTION_RELATIONS[sixFaceTwentyFourAngle][REVERSED_RELATION]);
                    // CORE_CELL.sixFace = newSixFace;
                    // CORE_CELL.faceDirection = newFaceDirection;
                    if (newSixFace !== CORE_CELL_SIX_FACE ||
                        newFaceDirection !== CORE_CELL_FOUR_DIRECTION) {
                        log(`[error]${newSixFace}vs${CORE_CELL_SIX_FACE},${newFaceDirection}vs${CORE_CELL_FOUR_DIRECTION}`);
                    }
                }
                if (SHOW_GET_CLONED_CUBE_BY_MANNER_INDEX_DETAIL) {
                ...
                sixFaces.forEach((face, faceIndex) => {
                    face.length = 0;
                    let find = false;
                    OLD_SIX_FACES.forEach((oldFaces) => {
                    ...
                    if (!find) {
                        ...
                        `${cube.no}.${mannerIndex.toString().padStart(2, "0")}.sixFaces[${faceIndex}] not found|${OLD_SIX_FACES.map((oldFaces) => {
                            const [rowIndex, colIndex] = oldFaces[0];
                            return `${rowIndex}${colIndex}${cells[rowIndex][colIndex].sixFace}`;
                        }).join("_")},all:${
                        // actCells.map((cell) => cell.sixFace).join("")
                        actCells.map((cell) => `${cell.rowIndex}${cell.colIndex}${cell.sixFace}`).join("_")}|${OLD_SIX_FACES.map((oldFaces) => oldFaces.map(([row1, col1, row2, col2]) => {
                            return `${row1}${col1}${cells[row1][col1].sixFace}${(typeof row2 !== "undefined" &&
                                typeof col2 !== "undefined")
                ...
                //   });
                // });
                const ORIGINAL_TWELVE_EDGES_PIECE_ARRAY = OLD_TWELVE_EDGES.filter((one) => one.pieces.length).map((one) => JSON.parse(JSON.stringify(one.pieces)));
                if (ORIGINAL_TWELVE_EDGES_PIECE_ARRAY.length) {
                    ...
                    ORIGINAL_TWELVE_EDGES_PIECE_ARRAY.forEach((pieces) => {
                        ...
                        //   twelveEdge,
                        // });
                        const goalTwelveEdge = twelveEdges[twelveEdge];
        ...
        log(`end: ${(new Date()).toLocaleString()}`);
        logUsedTime(`${STEP_AND_ROW_COUNT_FLAG}: Total`, performance.now() - DATE_BEGIN);
        const LOG_FILE_NAME_IN_CURRENT_PATH = `log_${STEP_FLAG}${MINI_OUTPUT_FLAG_POSTFIX}.txt`;
        const LOG_FILE_NAME_IN_OTHER_PATH = `${GOAL_FILE_TOP_PATH}log${logFilenamePostfix}.txt`;
        copySync(LOG_FILE_NAME, LOG_FILE_NAME_IN_CURRENT_PATH, OVER_WRITE_TRUE_FLAG);
        if (LOG_FILE_NAME_IN_CURRENT_PATH !== LOG_FILE_NAME_IN_OTHER_PATH) {
            copySync(LOG_FILE_NAME, LOG_FILE_NAME_IN_OTHER_PATH, OVER_WRITE_TRUE_FLAG);
    ...
    GLOBAL_LOG_CONTENT_ARRAY.push(`  end: ${(new Date()).toLocaleString()}`);
    GLOBAL_LOG_CONTENT_ARRAY.push(getUsedTimeString("Total", performance.now() - DATE_BEGIN));
    Deno.writeTextFileSync(`log_${STEP_FLAG}${MINI_OUTPUT_FLAG_POSTFIX}.txt`, GLOBAL_LOG_CONTENT_ARRAY.join("\n"));
 ...
 */
async function step4(ROW_COUNT_ARRAY, USE_LARGE_FILE = true, GOAL_FILE_TOP_PATH = "./", SOURCE_FILE_TOP_PATH = "./", SKIP_COUNT_CUBE = false, SKIP_COMPACT_LINE_INFO = false, SKIP_COMPACT_MANNER_FILES = false, OUTPUT_MANNER_BILL_FILE = false) {
    const GOAL_FILE_TOP_PATH_IS_NOT_CURRENT_PATH = GOAL_FILE_TOP_PATH.replace(/[.\/\\]/g, "").length;
    if (GOAL_FILE_TOP_PATH_IS_NOT_CURRENT_PATH) {
        ensureDirSync(GOAL_FILE_TOP_PATH);
        emptyDirSync(GOAL_FILE_TOP_PATH);
    ...
    GLOBAL_LOG_CONTENT_ARRAY.push(`begin: ${(new Date()).toLocaleString()}`);
    const DATE_BEGIN = performance.now();
    const CUBE_COUNT_WITH_ROWS_COUNT_ARRAY = [];
    const CUBE_COUNT_ARRAY = [];
    let cubeTotalCount = 0;
    for (let rowCountLoop = 2; rowCountLoop <= 5; ++rowCountLoop) {
        const ROW_COUNT = rowCountLoop;
        if (!ROW_COUNT_ARRAY.filter((value) => value === ROW_COUNT).length) {
            continue;
        ...
        const STEP_AND_ROW_COUNT_FLAG = `${STEP_FLAG}_${ROW_COUNT_FLAG}`;
        // const SOURCE_FILE_PATH = `${ROW_COUNT_FLAG_PREFIX}${SOURCE_FILE_TOP_PATH}/`;
        const SOURCE_FILE_PATH = `${SOURCE_FILE_TOP_PATH}manners_${ROW_COUNT_FLAG}/`;
        if (!existsSync(SOURCE_FILE_PATH)) {
            continue;
        }
        let logFilenamePostfix = "";
        ...
        await (async () => {
            ...
            async function compactMannerFiles() {
                ...
                const SOURCE_MANNER_FILE_PATH = `${SOURCE_FILE_TOP_PATH}manners_${ROW_COUNT_FLAG}/`;
                ...
                for await (const dirEntry of Deno.readDir(SOURCE_MANNER_FILE_PATH)) {
                    ...
                    // console.log("sort it");
                    Deno.writeTextFileSync(GOAL_FILE_NAME, `${existsSync(GOAL_FILE_NAME) ? "\n" : ""}${ARRAY.map(({ manner, cubeNoBill }) => `${manner}\t${cubeNoBill}`)
                        .join("\n")}`, APPEND_TRUE_FLAG);
                    // } else {
                    //   Deno.writeTextFileSync(
                    //     GOAL_FILE_NAME,
            ...
            async function compactMannerFilesForLargeFile() {
                ...
                const SOURCE_MANNER_FILE_PATH = `${SOURCE_FILE_TOP_PATH}manners${ROW_COUNT_FLAG_POSTFIX}/`;
                const MANNER_ARRAY = [];
                const CUBE_NO_ARRAY = [];
                let readedFileCount = 0;
                let mannerCount = 0;
                ...
                for (let outputIndex = 0; outputIndex < WRITE_TIMES; ++outputIndex) {
                    const QUARTER_CUBE_NO_ARRAY = CUBE_NO_ARRAY.splice(0, OUTPUT_COUNT_PER_TIME);
                    // if (outputIndex) {
                    // Deno.writeTextFileSync(
                    ...
                    // APPEND_TRUE_FLAG,
                    // );
                    Deno.writeTextFileSync(MANNER_GOAL_FILE_NAME, `${(outputIndex || existsSync(MANNER_GOAL_FILE_NAME)) ? "\n" : ""}${MANNER_ARRAY.splice(0, OUTPUT_COUNT_PER_TIME).map((MANNER, index) => `${MANNER}\t${QUARTER_CUBE_NO_ARRAY[index]}`).join("\n")}`, APPEND_TRUE_FLAG);
                }
            ...
            async function joinMannerFiles() {
                ...
                const GOAL_FILE_PATH = `${GOAL_FILE_TOP_PATH}joinedManners${ROW_COUNT_FLAG_POSTFIX}/`;
                ensureDirSync(GOAL_FILE_PATH);
                emptyDirSync(GOAL_FILE_PATH);
                const MANNER_ARRAY = [];
                let outputFileNo = 0;
            ...
            async function joinCubeFiles() {
                ...
                const SOURCE_MANNER_FILE_PATH = `${SOURCE_FILE_TOP_PATH}cubes${ROW_COUNT_FLAG_POSTFIX}/`;
                ...
                // const CUBE_GOAL_FILE_NAME =
                //   `${GOAL_FILE_TOP_PATH}${DEBUG_FILE_PREFIX}cubes.txt`;
                const CUBE_GOAL_FILE_NAME = `${GOAL_FILE_TOP_PATH}cubes${ROW_COUNT_FLAG_POSTFIX}.txt`;
            ...
            async function countCube() {
                ...
                const SOURCE_CUBE_FILE_PATH = `${SOURCE_FILE_TOP_PATH}cubes${ROW_COUNT_FLAG_POSTFIX}/`;
                let readedFileCount = 0;
                let cubeCount = 0;
    ...
    if (CUBE_COUNT_ARRAY.length) {
        ...
        Deno.writeTextFileSync(CUBE_COUNT_FILE_NAME, CUBE_COUNT_WITH_ROWS_COUNT_ARRAY.join("\n"));
    }
    GLOBAL_LOG_CONTENT_ARRAY.push(`  end: ${(new Date()).toLocaleString()}`);
    GLOBAL_LOG_CONTENT_ARRAY.push(getUsedTimeString("Total", performance.now() - DATE_BEGIN));
    Deno.writeTextFileSync(`log_${STEP_FLAG}.txt`, GLOBAL_LOG_CONTENT_ARRAY.join("\n"));
}
export async function main(options, ROW_COUNT_ARRAY = [2]) {
    const GLOBAL_LOG_CONTENT_ARRAY = [];
    GLOBAL_LOG_CONTENT_ARRAY.push(`begin: ${(new Date()).toLocaleString()}`);
    const DATE_BEGIN = performance.now();
    ...
    const { step1Option, step2Option, step3Option, step4Option, } = options;
    // useless:
    // console.log("\n");
    // log("\n");
    if (!step1Option?.skipped) {
        GLOBAL_LOG_CONTENT_ARRAY.push(`step1: ${(new Date()).toLocaleString()}`);
        step1(ROW_COUNT_ARRAY, step1Option?.GOAL_FILE_TOP_PATH, step1Option?.OUTPUT_CUT_MANNERS_ROW_BY_ROW);
    }
    if (!step2Option?.skipped) {
        GLOBAL_LOG_CONTENT_ARRAY.push(`step2: ${(new Date()).toLocaleString()}`);
        await step2(ROW_COUNT_ARRAY, step2Option?.GOAL_FILE_TOP_PATH, step2Option?.SOURCE_FILE_TOP_PATH, step2Option?.OUTPUT_ALONE_FIRST_CUBE, step2Option?.OUTPUT_CUBE_PASS_CHECK_FACES_LAYER_INDEX, step2Option?.OUTPUT_FIX_HIDDEN_PIECES, step2Option?.OUTPUT_MIDDLE_CUBE_TO_FIRST_NO, step2Option?.OUTPUT_CHECK_FACES_LAYER_INDEX_FAILED, step2Option?.OUTPUT_FIX_LONELY_FACE_OF_CUBE_AND_APPEND_IT, step2Option?.OUTPUT_FIX_LONELY_FACE_OF_CUBE);
    }
    if (!step3Option?.skipped) {
        GLOBAL_LOG_CONTENT_ARRAY.push(`step3: ${(new Date()).toLocaleString()}`);
        await step3(ROW_COUNT_ARRAY, step3Option?.GOAL_FILE_TOP_PATH, step3Option?.SOURCE_FILE_TOP_PATH, step3Option?.OUTPUT_FULL_CUBE, step3Option?.OUTPUT_READ_CUBE, step3Option?.SHOW_GET_CLONED_CUBE_BY_MANNER_INDEX_DETAIL, step3Option?.MINI_OUTPUT);
    }
    if (!step4Option?.skipped) {
        GLOBAL_LOG_CONTENT_ARRAY.push(`step4: ${(new Date()).toLocaleString()}`);
        await step4(ROW_COUNT_ARRAY, step4Option?.USE_LARGE_FILE, step4Option?.GOAL_FILE_TOP_PATH, step4Option?.SOURCE_FILE_TOP_PATH, step4Option?.SKIP_COUNT_CUBE, step4Option?.SKIP_COMPACT_LINE_INFO, step4Option?.SKIP_COMPACT_MANNER_FILES, step4Option?.OUTPUT_MANNER_BILL_FILE);
    }
    if (existsSync(LOG_FILE_NAME)) {
        Deno.removeSync(LOG_FILE_NAME);
    }
