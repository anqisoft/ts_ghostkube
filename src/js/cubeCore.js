...
var CellObject = /** @class */ (function () {
    ...
    CellObject.prototype.getConnectedCellIndexByRelation = function (relation) {
        var CELL_INDEX = this.cellIndex;
        switch (relation) {
...
var Cube = /** @class */ (function () {
    ...
    Cube.prototype.updateTwelveEdges = function () {
        this.twelveEdges.forEach(function (twelveEdge) {
            twelveEdge.canBeInserted = !!twelveEdge.pieces.length;
        });
        var twelveEdges = this.twelveEdges;
        var cells = this.cells;
        twelveEdges.forEach(function (oneEdge) {
            oneEdge.pieces.forEach(function (cellRowColIndex) {
                var rowIndex = cellRowColIndex[0], colIndex = cellRowColIndex[1];
                var pieceCell = cells[rowIndex][colIndex];
                var _a = pieceCell.relatedInformationWhenAdding, relatedRowIndex = _a.rowIndex, relatedColIndex = _a.colIndex;
    ...
    Cube.prototype.getCellArray = function () {
        var ARRAY = [];
        this.cells.forEach(function (rows) { return rows.forEach(function (cell) { return ARRAY.push(cell); }); });
        return ARRAY;
    };
    Cube.prototype.checkFacesLayerIndex = function () {
        var CELL_ARRAY = this.getCellArray();
        ...
        var _a = this, cells = _a.cells, sixFaces = _a.sixFaces;
        var SIX_FACE_CELL_ARRAY_ARRAY = sixFaces.map(function (faceInfo) {
            var ARRAY = [];
            faceInfo.forEach(function (_a) {
        ...
        for (var sixFaceIndex = 0; sixFaceIndex < exports.SixFaceCount; 
        // SIX_FACE_CELL_ARRAY_COUNT;
        ++sixFaceIndex) {
            var SIX_FACE_CELL_ARRAY = SIX_FACE_CELL_ARRAY_ARRAY[sixFaceIndex];
            var SIX_FACE_CELL_COUNT = SIX_FACE_CELL_ARRAY.length;
            var _loop_1 = function (faceCellIndex) {
                var FACE_CELL = SIX_FACE_CELL_ARRAY[faceCellIndex];
                // const FACE_CELL_SIX_FACE = FACE_CELL.sixFace;
                var FACE_CELL_LAYER_INDEX = FACE_CELL.layerIndex;
                var FACE_CELL_INNER_CELL_ARRAY = SIX_FACE_CELL_ARRAY.filter(function (cell) { return cell.layerIndex < FACE_CELL_LAYER_INDEX; });
                var FACE_CELL_INNER_CELL_COUNT = FACE_CELL_INNER_CELL_ARRAY.length;
                for (var faceCellBorderLineIndex = 0; faceCellBorderLineIndex < exports.FourDirectionCount; ++faceCellBorderLineIndex) {
                    if (FACE_CELL.borderLines[faceCellBorderLineIndex] !==
                        CellBorderLine.InnerLine) {
                        continue;
                    }
                    var FACE_CELL_CONNECTED_CELL = getCellByCellIndex(FACE_CELL.getConnectedCellIndexByRelation(faceCellBorderLineIndex));
                    if (!FACE_CELL_CONNECTED_CELL ||
                        FACE_CELL_CONNECTED_CELL.feature !== CellFeature.Face) {
                        continue;
                    }
                    var FACE_CELL_CONNECTED_CELL_SIX_FACE = FACE_CELL_CONNECTED_CELL.sixFace;
                    var FACE_CELL_CONNECTED_CELL_LAYER_INDEX = FACE_CELL_CONNECTED_CELL.layerIndex;
                    for (var faceCellInnerCellIndex = 0; faceCellInnerCellIndex < FACE_CELL_INNER_CELL_COUNT; ++faceCellInnerCellIndex) {
                        var FACE_CELL_INNER_CELL = FACE_CELL_INNER_CELL_ARRAY[faceCellInnerCellIndex];
                        for (var faceCellBorderLineIndex_1 = 0; faceCellBorderLineIndex_1 < exports.FourDirectionCount; ++faceCellBorderLineIndex_1) {
                            if (FACE_CELL_INNER_CELL.borderLines[faceCellBorderLineIndex_1] !==
                                CellBorderLine.InnerLine) {
                                continue;
                            }
                            var FACE_CELL_INNER_CELL_CONNECTED_CELL = getCellByCellIndex(FACE_CELL_INNER_CELL.getConnectedCellIndexByRelation(faceCellBorderLineIndex_1));
                            if (!FACE_CELL_INNER_CELL_CONNECTED_CELL ||
                                FACE_CELL_INNER_CELL_CONNECTED_CELL.feature !== CellFeature.Face) {
                                continue;
                            }
                            if (FACE_CELL_INNER_CELL_CONNECTED_CELL.sixFace !==
                                FACE_CELL_CONNECTED_CELL_SIX_FACE) {
                                continue;
                            }
                            if (FACE_CELL_INNER_CELL_CONNECTED_CELL.layerIndex >
            ...
            for (var faceCellIndex = 0; faceCellIndex < SIX_FACE_CELL_COUNT; ++faceCellIndex) {
                var state_1 = _loop_1(faceCellIndex);
    ...
    Cube.prototype.getCoreCellReserveRelatedInformation = function () {
        var cells = this.cells;
        var CORE_CELL = cells[this.coreRowIndex][this.coreColIndex];
        var FILTERED_BORDER_LINE_ARRAY = CORE_CELL.borderLines.filter(function (borderLine) { return borderLine === CellBorderLine.InnerLine; });
        // console.log(
        ...
        if (FILTERED_BORDER_LINE_ARRAY.length === 1) {
            for (var borderLineIndex = 0; borderLineIndex < exports.FourDirectionCount; ++borderLineIndex) {
                if (CORE_CELL.borderLines[borderLineIndex] !== CellBorderLine.InnerLine) {
                    continue;
                }
                var REVERSED_RELATION = getReversedRelation(borderLineIndex);
                var RELATION_CELL_CELL_INDEX = CORE_CELL
    ...
    Cube.prototype.updateIsValid = function () {
        ...
        var isValid = true;
        var _loop_2 = function (checkRowIndex) {
            if (!isValid) {
                return { value: void 0 };
            }
        ...
        for (var checkRowIndex = 0; checkRowIndex < rowCount; ++checkRowIndex) {
            var state_2 = _loop_2(checkRowIndex);
        ...
        if (SETTINGS.mustContainEveryColumn) {
            ...
            var _loop_3 = function (checkColIndex) {
                if (!isValid) {
                    return { value: void 0 };
                }
                isValid = actCells.filter(function (cell) {
            ...
            for (var checkColIndex = 0; checkColIndex < colCount; ++checkColIndex) {
                var state_3 = _loop_3(checkColIndex);
        ...
        actCells.forEach(function (cell) {
            var rowIndex = cell.rowIndex, colIndex = cell.colIndex, borderLines = cell.borderLines, sixFace = cell.sixFace, twelveEdge = cell.twelveEdge;
            var sixFaceIndex = 0;
            switch (borderLines.filter(function (borderLine) {
                ...
                case 1:
                    ...
                    if (cell.relatedInformationWhenAdding.rowIndex === -1) {
                        ...
                        //   relation = 4 - relation;
                        // }
                        var relation = getReversedRelation(RELATION_CELL.relatedInformationWhenAdding.relation);
...
var SimpleCell = /** @class */ (function () {
    ...
    SimpleCell.prototype.clone = function () {
        var RESULT = new SimpleCell(this.rowIndex, this.colIndex, this.cellIndex);
        RESULT.addOrder = this.addOrder;
        RESULT.relatedInformationWhenAdding = this.relatedInformationWhenAdding;
        RESULT.feature = this.feature;
        RESULT.sixFace = this.sixFace;
...
var SimpleCube = /** @class */ (function () {
    ...
    SimpleCube.prototype.count = function () {
        ...
        function getIsValid() {
            var _loop_4 = function (checkRowIndex) {
            ...
            for (var checkRowIndex = 0; checkRowIndex < rowCount; ++checkRowIndex) {
                var state_4 = _loop_4(checkRowIndex);
                if (typeof state_4 === "object")
                    return state_4.value;
            }
            if (SETTINGS.mustContainEveryColumn) {
                var _loop_5 = function (checkColIndex) {
                    if (!actCells.filter(function (cell) {
                        return cell.colIndex === checkColIndex;
                    }).length) {
                        return { value: false };
                ...
                for (var checkColIndex = 0; checkColIndex < colCount; ++checkColIndex) {
                    var state_5 = _loop_5(checkColIndex);
...
function getCubeFromJson(json) {
    ...
    // coreColIndex,
    // cells,
    sixFaces = CUBE.sixFaces, twelveEdges = CUBE.twelveEdges, isValid = CUBE.isValid;
...
function getReversedRelation(relation) {
    return relation % 2 + 2 * (1 - Math.floor(relation / 2));
}
exports.getReversedRelation = getReversedRelation;
/*
